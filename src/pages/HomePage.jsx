...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const scrollStep = () => {
        // Stop if not mobile layout (check via window width or css check)
        ...
        scrollContainer.scrollTo({
            left: nextScrollPos, // RTL direction handles this logic usually, but keep eye on it
            behavior: 'smooth'
        });
    ...
    const interval = setInterval(scrollStep, 3500); // Move every 3.5 seconds
  ...
  return (
    <div className="animate-fadeIn">
      ...
      {/* 1. Hero Section */}
      <section className="relative h-[85vh] lg:h-[95vh] flex items-center justify-center overflow-hidden bg-dark">
        {HERO_SLIDES.map((slide, index) => (
          ...
          >
            ...
            <div className="absolute inset-0 z-20 flex flex-col justify-center items-center text-center px-4 container mx-auto">
              ...
              <div className="mb-6 lg:mb-8 animate-slideUp">
                <h2 className="text-4xl lg:text-7xl font-black tracking-tight drop-shadow-2xl" style={{ fontFamily: "'Oswald', sans-serif" }}>
                  <span className="text-white">OFF FIRE</span> <span className="text-cta">ONLINE</span>
                </h2>
              </div>
              
              <h1 className="text-xl md:text-4xl lg:text-6xl font-extrabold text-text-main mb-4 lg:mb-8 leading-tight max-w-5xl animate-slideUp delay-100 drop-shadow-lg">
                {slide.title}
              </h1>
              <p className="text-base md:text-xl lg:text-2xl text-text-sub mb-8 lg:mb-12 max-w-3xl animate-slideUp delay-200 font-medium leading-relaxed px-2">
                {slide.subtitle}
              </p>
              <div className="animate-slideUp delay-300 w-full px-4 flex justify-center">
                 <Link to="/contact" className="w-full sm:w-auto">
                    <Button primary className="text-base lg:text-xl px-8 lg:px-12 py-3 lg:py-4 shadow-cta/20 shadow-xl rounded-xl w-full sm:w-auto">
                      {slide.cta}
                    </Button>
                 </Link>
              </div>
      ...
      <section className="py-12 lg:py-28 bg-dark">
        <div className="container mx-auto px-4 lg:px-6">
          <div className="text-center mb-8 lg:mb-20">
            <h2 className="text-xl md:text-5xl font-extrabold text-text-main mb-3 lg:mb-6 relative inline-block">
            ...
            <p className="text-text-sub text-sm lg:text-xl mt-3 lg:mt-6 max-w-3xl mx-auto">تبدأ كل منشأة بطلب واضح، لكن الدوامة تبدأ سريعاً.</p>
          </div>
            ...
            {PAIN_POINTS.map((item, idx) => {
              ...
              return (
                <div key={idx} className="bg-card p-4 lg:p-10 rounded-xl lg:rounded-2xl border border-white/5 hover:border-cta transition-all group">
                  <div className="text-cta mb-3 lg:mb-8 bg-dark w-10 h-10 lg:w-20 lg:h-20 rounded-lg lg:rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform shadow-lg mx-auto lg:mx-0">
                    <Icon size={20} className="lg:w-10 lg:h-10" />
                  </div>
                  <h3 className="text-sm lg:text-2xl font-bold text-text-main mb-2 lg:mb-4 text-center lg:text-right">{item.title}</h3>
                  <p className="text-text-sub leading-snug text-xs lg:text-lg text-center lg:text-right opacity-80">{item.desc}</p>
                </div>
              );
            })}
          </div>
        ...
        <div className="container mx-auto px-4 lg:px-6 relative z-10">
          <div className="grid md:grid-cols-2 gap-8 lg:gap-20 items-center">
            ...
            <div className="order-2 md:order-1">
              <span className="text-primary font-bold mb-2 lg:mb-4 block uppercase tracking-wider text-sm lg:text-lg border-l-4 border-primary pl-3">الحل الذكي</span>
              <h2 className="text-2xl md:text-6xl font-extrabold text-text-main mb-6 lg:mb-10 leading-tight">
              ...
              <ul className="space-y-4 lg:space-y-8 mb-8 lg:mb-12">
                {FEATURES.map((feat, i) => (
                  <li key={i} className="flex items-start gap-3 lg:gap-5 group">
                    <div className="bg-dark p-1.5 lg:p-2 rounded-lg border border-white/5 group-hover:border-primary/50 transition-colors mt-1">
                        <CheckCircle className="text-green-500 flex-shrink-0 w-4 h-4 lg:w-6 lg:h-6" />
                    </div>
                    <div>
                      <h4 className="font-bold text-text-main text-sm lg:text-xl mb-0.5 lg:mb-2 group-hover:text-primary transition-colors">{feat.title}</h4>
                      <p className="text-text-sub text-xs lg:text-lg">{feat.desc}</p>
                    </div>
                  </li>
                ))}
              </ul>
              <Link to="/contact">
                <Button primary className="w-full lg:w-auto text-base lg:text-xl px-10 py-3 lg:py-4 shadow-lg shadow-primary/20">ابدأ رحلتك الآن</Button>
              </Link>
            </div>
      ...
      <section className="py-12 lg:py-28 bg-dark">
        <div className="container mx-auto px-4 lg:px-6">
          <div className="text-center mb-8 lg:mb-20">
            ...
            <p className="text-text-sub text-sm lg:text-xl">تحت سقف واحد.. قارن، تأكد، ونفذ.</p>
          </div>
          ...
          <div className="relative">
            ...
            >
                ...
                <div key={service.id} className="bg-card p-5 lg:p-8 rounded-2xl border border-white/5 min-w-[85vw] sm:min-w-[45vw] lg:min-w-0 snap-center hover:border-primary transition-all group">
                    <div className="mb-4 lg:mb-8 overflow-hidden rounded-xl h-40 lg:h-48 shadow-lg relative">
                        ...
                        <div className="absolute bottom-2 left-2 bg-dark/70 text-white text-[10px] px-2 py-1 rounded lg:hidden flex items-center gap-1">
                            اسحب للمزيد <ArrowLeft size={10} />
                        </div>
                    </div>
                    <h3 className="text-lg lg:text-2xl font-bold text-text-main mb-2 lg:mb-3">{service.title}</h3>
                    <p className="text-text-sub text-sm lg:text-lg mb-4 leading-relaxed line-clamp-2 lg:line-clamp-none">{service.desc}</p>
                    <Link to="/services" className="text-primary text-sm lg:text-lg font-bold flex items-center gap-2">
                        تفاصيل أكثر <ArrowLeft size={16} className="lg:w-5 lg:h-5" />
                    </Link>
                </div>
                ))}
            </div>
      ...
      <section className="py-12 lg:py-28 bg-darker">
        <div className="container mx-auto px-4 lg:px-6">
          <div className="grid lg:grid-cols-2 gap-10 lg:gap-20">
            ...
            {/* من نخدم */}
            <div>
                <h3 className="text-xl lg:text-3xl font-bold text-text-main mb-6 lg:mb-10 border-r-4 border-cta pr-3 lg:pr-6">من نخدم؟</h3>
                <div className="grid grid-cols-2 gap-3 lg:gap-6">
                    {TARGET_AUDIENCE.map((aud, i) => (
                        <div key={i} className="bg-dark p-4 lg:p-8 rounded-xl lg:rounded-2xl border border-white/5">
                            <h4 className="font-bold text-primary text-sm lg:text-xl mb-1 lg:mb-3">{aud.title}</h4>
                            <p className="text-text-sub text-[10px] lg:text-base leading-tight">{aud.desc}</p>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="mt-8 lg:mt-0">
                <h3 className="text-xl lg:text-3xl font-bold text-text-main mb-6 lg:mb-10 border-r-4 border-primary pr-3 lg:pr-6">لماذا نحن خيارك الذكي؟</h3>
                <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 lg:gap-6">
                    {WHY_US.slice(0,4).map((item, i) => (
                        <div key={i} className="flex flex-col lg:flex-row items-center lg:items-start gap-2 lg:gap-6 bg-dark p-4 lg:p-6 rounded-xl border border-white/5 text-center lg:text-right">
                            <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-sm lg:text-lg shrink-0">{i+1}</div>
                            <div>
                                <h4 className="font-bold text-text-main text-sm lg:text-xl mb-1">{item.title}</h4>
                                <p className="text-text-sub text-[10px] lg:text-base leading-tight">{item.desc}</p>
                            </div>
                        </div>
                    ))}
                </div>
      ...
      <section className="py-12 lg:py-28 bg-dark">
        <div className="container mx-auto px-4 lg:px-6 max-w-4xl">
          <div className="text-center mb-8 lg:mb-16">
            <h2 className="text-2xl lg:text-4xl font-bold text-text-main">الأسئلة الشائعة</h2>
          </div>
            ...
            {FAQ.map((item, idx) => (
                <div key={idx} className="bg-card rounded-xl lg:rounded-2xl border border-white/5 overflow-hidden">
                    <button 
                        ...
                        className="w-full flex justify-between items-center p-4 lg:p-6 text-right font-bold text-sm lg:text-xl text-text-main hover:bg-white/5 transition-colors"
                    >
                        {item.q}
                    ...
                    <div className={`transition-all duration-300 ${openFaq === idx ? 'max-h-48 p-4 lg:p-6 pt-0' : 'max-h-0 overflow-hidden'}`}>
                        <p className="text-text-sub text-xs lg:text-lg leading-relaxed border-t border-white/5 pt-3 lg:pt-4">{item.a}</p>
                    </div>
                </div>
            ))}
          </div>
      ...
      <section className="py-12 lg:py-24 bg-gradient-to-r from-primary/10 to-cta/10 border-t border-white/10">
        <div className="container mx-auto px-4 lg:px-6 text-center">
            ...
            <p className="text-sm lg:text-2xl text-text-sub mb-6 lg:mb-10 max-w-3xl mx-auto">OFF FIRE ONLINE تمنحك القدرة على اتخاذ قرار سريع وآمن.</p>
            <div className="flex flex-col sm:flex-row gap-3 lg:gap-6 justify-center">
                <Link to="/contact"><Button primary className="w-full sm:w-auto text-base lg:text-xl px-8 py-3 lg:px-12 lg:py-4">اطلب عروض أسعار</Button></Link>
                <Link to="/contact"><Button className="w-full sm:w-auto text-base lg:text-xl px-8 py-3 lg:px-12 lg:py-4">تحدث مع مستشارنا</Button></Link>
            </div>
        </div>
      </section>
    </div>
